...
import { ClipboardList, Workflow, Clock, TrendingUp, CheckSquare, BarChart3, ArrowRight, CheckCircle, Star, Award, Download, Loader2 } from 'lucide-react';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { error } = await supabase.from('leads').insert({
        ...
        requirement: `营销SOP白皮书下载 - 邮箱: ${formData.email}`,
        status: 'pending',
      });
          ...
          <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            >
              <div className="text-center mb-8">
                <h2 className="text-3xl font-bold text-sky-900 mb-4">获取《营销SOP私域运营白皮书》</h2>
                <p className="text-sky-700/70 max-w-xl mx-auto">深入了解标准化运营流程最佳实践，包含新东方、招商银行等头部案例拆解</p>
              </div>
              ...
              {isSuccess ? (
                ...
                >
                  ...
                  <h3 className="text-2xl font-bold text-sky-900 mb-2">提交成功！</h3>
                  <p className="text-sky-700 mb-4">白皮书将发送至您的邮箱，请查收</p>
                  <p className="text-sky-500 text-sm">3秒后自动跳转到试用页面...</p>
                </motion.div>
              ) : (
                <form className="max-w-md mx-auto space-y-4" onSubmit={handleSubmit}>
                  <input
                    ...
                    className="w-full px-4 py-3 rounded-xl border border-sky-200 focus:border-sky-500 focus:ring-2 focus:ring-sky-200 outline-none transition-all"
                    required
                  ...
                  <button
                    type="submit"
                    disabled={isSubmitting}
                    className="w-full py-4 bg-gradient-to-r from-sky-500 to-cyan-500 text-white font-semibold rounded-xl hover:from-sky-600 hover:to-cyan-600 transition-all duration-300 shadow-lg disabled:opacity-70 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                  >
                    {isSubmitting ? (
                      <>
